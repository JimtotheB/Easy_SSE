...
module.exports = function expressRoutes(app){
  ...
  router.get('/trigger', routes.trigger);
  router.get('/complex', routes.complex)
  router.get('/event-stream', routes.events);
  app.use('/', router);
...
routes.events = function(req, res){
  ...
  var handleComplex = function(data){
    ...
    //If you are using the compression middleware with express
    //You will need to flush the writes to the browser.
    //res.flush()
  };
